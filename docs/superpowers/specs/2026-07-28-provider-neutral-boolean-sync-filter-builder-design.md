...
- An existing editor sends its positive safe `batchId` with refresh. Canonical validation may retain unresolved pairs only when that church/provider-scoped batch already persists the exact pair in its active filter or draft. Missing/new-batch identity remains strict. Shared cache/response metadata stays provider-backed only; the client projects missing persisted selections from the batch config as unavailable/removable instead of turning them into canonical options. Refresh re-checks the target batch's schema, revision, and canonical active/draft digests immediately before validation and cache replacement, returning a typed stale response without replacing the old cache if it changed during provider fetch.
