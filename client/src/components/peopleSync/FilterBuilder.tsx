...
type PendingConstruction = { kind: 'branch' } | { kind: 'group'; branchIndex: number };
...
function PendingPicker({ pending, metadata, config, disabled, onCancel, onInclude, onNot }: {
...
export default function FilterBuilder({ metadata, value, onChange, disabled = false }: FilterBuilderProps) {
  ...
  useEffect(() => { if (!restoreKey) return; notButtons.current.get(restoreKey)?.focus(); setRestoreKey(null); }, [config, restoreKey]);
  return <div className="space-y-4">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">Qualification rules</h3><p className="text-sm text-gray-600 dark:text-gray-300">Every bracket in a branch must match; any branch can qualify someone.</p></div>{config.branches.length === 0 && !pending ? <button type="button" disabled={disabled} onClick={() => setPending({ kind: 'branch' })} aria-label="Add Branch 1" className="rounded-md bg-green-600 px-3 py-2 text-sm font-medium text-white disabled:opacity-50">Add Branch 1</button> : null}</div>
