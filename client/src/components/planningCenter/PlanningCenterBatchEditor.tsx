...
export default function PlanningCenterBatchEditor({ batch: initialBatch, onSaved, onCancel }: Props) {
  ...
  const sourceChanged = !!currentBatch && !!selection && (!currentBatch.source || selection.sourceKind !== currentBatch.source.kind || selection.sourceExternalId !== currentBatch.source.externalId);
  ...
  return <div className="space-y-5 rounded-md border border-gray-200 p-4 dark:border-gray-700">
    ...
    <div><p className="mb-2 text-sm font-medium">Schedule</p><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={scheduleEnabled} onChange={e => setScheduleEnabled(e.target.checked)} />Runs automatically</label>{scheduleEnabled && <div className="mt-2 flex flex-wrap gap-2"><select aria-label="Schedule frequency" value={scheduleFrequency} onChange={e => changeFrequency(e.target.value as 'daily' | 'weekly' | 'monthly')}><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option></select>{scheduleFrequency === 'weekly' && <select aria-label="Schedule day" value={scheduleDay} onChange={e => setScheduleDay(Number(e.target.value))}>{['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'].map((day, i) => <option key={day} value={i}>{day}</option>)}</select>}{scheduleFrequency === 'monthly' && <select aria-label="Schedule day" value={scheduleDay} onChange={e => setScheduleDay(Number(e.target.value))}>{Array.from({ length: 31 }, (_, i) => i + 1).map(day => <option key={day} value={day}>{ordinalDay(day)}</option>)}</select>}</div>}</div>
